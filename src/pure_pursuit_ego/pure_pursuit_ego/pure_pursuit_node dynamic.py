...
class PurePursuit(Node):
    ...
    def __init__(self):
        ...
        self.marker_pub = self.create_publisher(Marker, '/occupancy_grid', 10)
        ...
        # dist = np.linalg.norm(np.diff(waypoint[:, :2]), axis=1)
        # time = (dist/a)**(1/b)

        # fit splines
        tck, u = splprep(waypoint.T, s=0, per=True)
        dense_waypoints = np.array(splev(np.linspace(0, 1, 1000), tck))
        ...
        # trajectory_position = dense_waypoints[:2, :].T
        # trajectory_time = np.diff(s) / dense_waypoints[2, :-1]
        # cumulative_time = np.cumsum(trajectory_time)
        # cumulative_time = np.insert(cumulative_time, 0, 0)
        # lookahead_trajectory_position = np.empty(trajectory_position.shape)
        # for i, t in enumerate(cumulative_time):
        #     j = np.searchsorted(cumulative_time, (t + position_lookahead_time)%cumulative_time[-1])
        #     lookahead_trajectory_position[i, :] = trajectory_position[j, :]

        # trajectory_position = trajectory_position[:-1]
        # lookahead_trajectory_position = lookahead_trajectory_position[:-1]

        # self.target = np.hstack((lookahead_trajectory_position, dense_waypoints[2, :-1].reshape(-1,1)))

        self.target = dense_waypoints[2, :-1]

        # # upsample the trajectory position and target
        # x = np.linspace(0, len(trajectory_position)-1, num=1000)
        # f = interp1d(range(len(trajectory_position)), trajectory_position, axis=0)
        # trajectory_position = f(x)
    ...
    def scan_sub_callback(self, scan_msg):
        ...
        # lookahead_position = self.target[neigh_ind,0:2]
        ...
        trajectory_cumsum = self.trajectory_cumsum/max(self.lookahead_speed_min, speed)
    ...
    def get_caution_gain(self, obs_in_world_frame):
        ...
        # # get the distance from the mean to the trajectory
        # distance_from_trajectory = self.kn_regressor.kneighbors(mean_obs_in_world_frame[:2].reshape(1, -1))[0][0][0]
        # distance_from_trajectory = min(distance_from_trajectory, self.lateral_caution_distance)
        ...
        # caution_gain_from_trajectory = max(distance_from_trajectory-self.lateral_stop_distance, 0) / self.lateral_caution_distance
        # caution_gain = max(caution_gain_from_ego, caution_gain_from_trajectory)

        # log the gains
        # self.get_logger().info('Caution Gain from Ego: %f, Caution Gain from Trajectory: %f' % (caution_gain_from_ego, caution_gain_from_trajectory))
